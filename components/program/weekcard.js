...
const CourseCard = ({data, currentWeek, currentDay}) => {
  let isLock;
  if(currentWeek<data.week) isLock=true
  let percentage;
  ...
  return (
    <div className=" rounded-lg p-6 shadow-md cursor-pointer" style={{backgroundColor: data.bgcolor, cursor: isLock?'not-allowed':'pointer'}}>
      <div className="flex justify-between items-start">
        ...
        <div className="bg-purple-100 p-2 rounded-full">
          {
            isLock ? <i className="fas fa-lock text-lg" style={{color: data.color}}></i> : <i className="fas fa-mobile-alt text-lg" style={{color: data.color}}></i>
          }
      ...
      <div className="mt-4">
        ...
        <div className="mt-1 h-2 bg-purple-200 rounded-full relative">
          <div className="absolute top-0 left-0 h-2 bg-purple-500 rounded-full" style={{ width: `${percentage.toFixed()}%`, backgroundColor: data.color }}></div>
        </div>
